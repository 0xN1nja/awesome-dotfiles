...
function parseSubmissionDate(date: string, file: string): number {
  const parsed = Date.parse(`${date}T00:00:00Z`);
  if (Number.isNaN(parsed)) {
    throw new Error(
      `data/github/${file}: invalid or missing "date" (expected "YYYY-MM-DD"), got ${JSON.stringify(date)}`,
    );
  }
    ...
    .flatMap((f) => {
      ...
      return rices.map(
        (rice, i): RiceDetail => ({
          ...
          created_utc: parseSubmissionDate(rice.date, f),
          source: "github",
